...
class _WebDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        _Sidebar(meta: meta, email: user.email),
        Expanded(
          child: Column(
            children: [
              _WebTopBar(email: user.email, meta: meta),
              Expanded(
                child: _ContentArea(role: role, meta: meta, isWeb: true),
              ),
            ],
          ),
        ),
...
class _Sidebar extends StatelessWidget {
  final _RM meta;
  final String email;
  const _Sidebar({required this.meta, required this.email});

  @override
  Widget build(BuildContext context) {
    final items = [
      _NavItem(Icons.dashboard_rounded, 'Dashboard', true),
      _NavItem(Icons.directions_car_rounded, 'Vehicles', false),
      _NavItem(Icons.person_pin_rounded, 'Drivers', false),
      _NavItem(Icons.navigation_rounded, 'Trips', false),
      _NavItem(Icons.build_circle_rounded, 'Maintenance', false),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...items.map(
                    (item) => _NavTile(item: item, accent: meta.accent),
                  ),
                ],
              ),
            ),
...
class _NavTile extends StatefulWidget {
  final _NavItem item;
  final Color accent;
  const _NavTile({required this.item, required this.accent});
...
class _WebTopBar extends StatelessWidget {
  final String email;
  final _RM meta;
  const _WebTopBar({required this.email, required this.meta});
